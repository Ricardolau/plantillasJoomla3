 ...
 * @version $Id: order.php 9483 2017-03-19 09:31:46Z Milbo $
 */
...
<div><h3>Listado productos pedidos</h3></div>
<table width="100%">
	<tr>
		<td colspan="2">
		...
		foreach ($this->orderdetails['items'] as $item) { ?>
			...
			<tr valign="top" <?php echo $lId?>><?php /*id="showItem_<?php echo $item->virtuemart_order_item_id; ?>" data-itemid="<?php echo $item->virtuemart_order_item_id; ?>">*/ ?>
				...
				<td>
					...
					<?php 
					// Array de atributos que tiene el producto
					$Attributos = array();
					$i = 0;
					if (strlen($item->product_attribute)> 0){
						foreach (json_decode($item->product_attribute) as $clave => $valor) {
							foreach ($item->customfields as $custom){
								if ( $clave == $custom->virtuemart_custom_id && $valor == $custom->virtuemart_customfield_id) {
									$i++;
									$Attributos[$i]['custom_id'] = $clave;
									$Attributos[$i]['customfield_id'] = $valor;
									$Attributos[$i]['custom_title'] = $custom->custom_title;
									$Attributos[$i]['customfield_value'] = $custom->customfield_value;
									echo '<br/>';
									echo '<strong class="atributo">'.$Attributos[$i]['custom_title'].':</strong>';
									echo '<span class="atributo">'.$Attributos[$i]['customfield_value'].':</span>';
