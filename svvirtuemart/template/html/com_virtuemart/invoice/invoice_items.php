...
<?php  /* Mostramos cupon y descuento si existe */
...
<?php  } ?>
	...
	<?php 	/* Comentamos ya que pensamos que no nos hace falta
			 * Nos genera lineas iva a mayores ... 
			 * 
	...
	<tr>
		<td align="right" class="pricePad" colspan="6">
			<?php //echo $this->orderDetails['shipmentName']
				echo 'Gastos de envio:';
			 ?>
		</td>

		<?php if ( VmConfig::get('show_tax')) { ?>
		<td align="right"><span class='priceColor2'>
			<?php /* Importe impuesto gastos forma envio... */ 
			echo $this->currency->priceDisplay($this->orderDetails['details']['BT']->order_shipment_tax, $this->currency) ?></span> 
		</td>
		<?php } ?>
		<td align="right"></td>
		<td align="right">
			<?php /* Importe total envio con impuesto. */ 
			echo $this->currency->priceDisplay($this->orderDetails['details']['BT']->order_shipment + $this->orderDetails['details']['BT']->order_shipment_tax, $this->currency); ?></td>
	</tr>

	<tr>
		<td align="right" class="pricePad" colspan="6">
		<?php // Mostramos descripción con imagen de forma pago 
		echo 'Gastos /Descuento por forma pago:';
		//$this->orderDetails['paymentName'];
		?>
		</td>

		<?php if ( VmConfig::get('show_tax')) { ?>
		<td align="right"><span class='priceColor2'><?php echo $this->currency->priceDisplay($this->orderDetails['details']['BT']->order_payment_tax, $this->currency)
		 ?></span> </td>
		<?php } ?>
		<td align="right"></td>
	...
	<tr>
		<td align="right" class="pricePad" colspan="7"><strong><?php echo vmText::_('COM_VIRTUEMART_CART_SUBTOTAL') ?></strong></td>
		<td align="right" class="pricePad" ><strong><?php echo vmText::_('COM_VIRTUEMART_ORDER_PRINT_PRODUCT_TAX'); ?></strong></td>
		<td align="right" class="pricePad" ><strong><?php echo vmText::_('COM_VIRTUEMART_ORDER_PRINT_TOTAL'); ?></strong></td>

	</tr>
	<tr>	
		<?php 
		// Si hubiera descuento tambien deberíamos mostralos.... pero no lo muestro
		...
		<td align="right" colspan="7">
			...
			<?php 
			// Deberíamos buscar la variable subtotal ( Importe base antes de impuestos) pero como no tengo lo calculo
			$subtotal = $this->orderDetails['details']['BT']->order_total-$this->orderDetails['details']['BT']->order_billTaxAmount;
			// Utilizamos $this->currency para que redondé...
			echo $this->currency->priceDisplay($subtotal,$this->currency);
			 ?>
			</span>
		</td>
		<?php } ?>
