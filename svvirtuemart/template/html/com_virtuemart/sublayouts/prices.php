...
/* Obtengo precios que me interesan para este cliente*/
$PrecioIva = number_format($product->prices['basePriceWithTax'], 2, ',', ' '); // Doy formato al Precio con Iva...
$PrecioFinal = number_format($product->prices['product_override_price'], 2, ',', ' '); // Precio ya con descuento.. 
number_format($product->prices['basePriceWithTax'], 2, ',', ' '); // Doy formato al numero...
/* El precio final realmente es con descuento si hay..
 * si no tiene descuento es 0 , por lo que debo igualarlo.
 * */
if ( strval($PrecioFinal) == 0 ){
...
<div class="product-price" id="productPrice<?php echo $product->virtuemart_product_id ?>">
	...
	if ($product->prices['salesPrice']<=0 and VmConfig::get ('askprice', 1) and isset($product->images[0]) and !$product->images[0]->file_is_downloadable) {
		...
		/* Ahora vamos a mostrar precios 
		 * si el Precio con Iva y Final solo mostramos el FINAL
		 * */ 
		if ( $PrecioIva == $PrecioFinal ){
			echo '<div class="Precio">';
			echo $PrecioFinal.'<span >€</span>'; // Esto debería ser una constante
			echo '</div>';
		} else {
		echo '<span class="PrecioTachado">'.$PrecioIva.'</span>';// Esto debería ser una constante
		echo '<div class="Precio">';
		echo $PrecioFinal.'<span >€</span>'; // Esto debería ser una constante
		echo '</div>';
		}	
